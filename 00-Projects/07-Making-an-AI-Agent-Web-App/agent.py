...
# La sesión es la misma que hay en app.py.
def get_location():
    ...
    print("lat lon from get_location", lat, lon)
    ...
    data = response.json()
    print(data)
    city = data['address'].get('city', data['address'].get('town', 'Unknows'))
...
3. If the user's location is in US, Liberia or Burma, only mention the temperature in Fahrenheit. Otherwise, only mention the temperature in Celsius.
